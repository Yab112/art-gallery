...
export function WithdrawalSection() {
  ...
  const { data: ibansData } = useQuery({
    queryKey: ['artist-ibans'],
    queryFn: async () => {
      const response = await axiosAuth.get('/artworks/my-artworks');
      const artworks = response.data?.artworks ?? response.data ?? [];
      const uniqueIbans = [...new Set(artworks.map((a: any) => a.iban).filter(Boolean))];
      return uniqueIbans;
    },
  ...
  const isLoading = isLoadingWithdrawals || isLoadingEarnings;
          ...
          <div className="relative inline-block group">
            ...
            >
              ...
              <form onSubmit={handleWithdrawal} className="space-y-4">
                ...
                <div>
                  <Label htmlFor="iban">IBAN / Payment Account</Label>
                  {ibans.length > 0 ? (
                    <select
                      id="iban"
                      value={iban}
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {ibans.length > 0 
                      ? "Select from your saved payment accounts or enter a new one"
                      : "Enter the IBAN where you want to receive the payment"}
                  </p>
                </div>
                          ...
                          {withdrawal.paypalStatus ? (
                            <span
                              className={`px-3 py-1 rounded-full text-xs font-medium ${
                                withdrawal.paypalStatus === 'SUCCESS' || withdrawal.paypalStatus === 'COMPLETED'
