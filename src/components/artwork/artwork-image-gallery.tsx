...
export const ArtworkImageGallery: React.FC<ArtworkImageGalleryProps> = ({ photos, alt }) => {
    ...
    return (
        <>
            <div className="space-y-3">
                {/* Main Image */}
                <div className="group relative aspect-[4/5] overflow-hidden rounded-lg bg-gray-100">
                    {imageErrors.has(currentIndex) || !currentPhoto ? (
                        <div className="flex h-full w-full items-center justify-center bg-gray-200">
                            <div className="text-center">
                                ...
                                <p className="mt-2 text-gray-500 text-xs">Image not available</p>
                            </div>
                        </div>
                    ) : (
                        <>
                            ...
                            {hasMultiplePhotos && (
                                <>
                                    <Button
                                        variant="secondary"
                                        size="icon"
                                        className="-translate-y-1/2 absolute top-1/2 left-2 bg-white/90 opacity-0 shadow-md transition-opacity hover:bg-white group-hover:opacity-100"
                                    ...
                                    <Button
                                        variant="secondary"
                                        size="icon"
                                        className="-translate-y-1/2 absolute top-1/2 right-2 bg-white/90 opacity-0 shadow-md transition-opacity hover:bg-white group-hover:opacity-100"
                                        onClick={handleNext}
                            ...
                            {hasMultiplePhotos && (
                                <div className="absolute top-2 right-2 rounded-md bg-black/50 px-2 py-1 font-medium text-white text-xs">
            ...
            {isModalOpen && currentPhoto && !imageErrors.has(currentIndex) && (
                <ImageModal src={currentPhoto} onClose={() => setIsModalOpen(false)} />
            )}
        </>
    )
}
