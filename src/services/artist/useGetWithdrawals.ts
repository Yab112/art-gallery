...
const fetchWithdrawals = async (page = 1, limit = 20): Promise<WithdrawalsResponse> => {
    const response = await axios.get(`${getApiBaseUrl()}/api/artist/withdrawals`, {
        params: {
            page,
            limit
        },
...
export const useGetWithdrawals = (page = 1, limit = 20) => {
    ...
    const query = useQuery({
        ...
        refetchInterval: (query) => {
            ...
            const hasProcessing = data?.data?.some(
                (w) => w.status === "PROCESSING" || w.status === "INITIATED"
            )
    ...
    useEffect(() => {
        ...
        if (currentData?.data) {
            ...
            const currentCompletedIds = new Set(
                currentData.data.filter((w) => w.status === "COMPLETED").map((w) => w.id)
            )
