...
export const useUpdateArtwork = () => {
  const { mutateAsync, isPending } = useMutationFunc<UpdateArtworkResponse, Partial<CreateArtworkDto>>({
    onSuccess: () => {
      toast.success("Artwork updated successfully");
    },
    onError: (error) => {
      toast.error("Failed to update artwork: " + (error?.message || "An unexpected error occurred"));
    },
