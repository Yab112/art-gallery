...
export const useCreateCollection = () => {
  const { mutateAsync, isPending } = useMutationFunc<CreateCollectionResponse, CreateCollectionDto>({
    onSuccess: (data) => {
      toast.success("Collection created successfully");
      return data; // Return the response so we can access collection.id
    },
    onError: (error) => {
      toast.error("Failed to create collection: " + (error?.message || "An unexpected error occurred"));
    },
