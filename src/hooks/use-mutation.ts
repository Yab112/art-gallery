...
interface UseMutationFuncOptions<TResponseData, TRequestBody, TError = ApiErrorResponse> {
    onSuccess?: (data: TResponseData, options: MutationOptions<TRequestBody>) => void
    onError?: (
        error: TError,
        options?: MutationOptions<TRequestBody>,
        context?: { previousData: unknown }
    ) => void
    onMutate?: (oldData: unknown, options: MutationOptions<TRequestBody>) => unknown
...
const useMutationFunc = <TResponseData, TRequestBody, TError = ApiErrorResponse>(
    ...
    >({
        ...
        onMutate: (mutationOptions) => {
            if (!options?.queryKey || !options?.onMutate) return { previousData: undefined }
            queryClient.cancelQueries({ queryKey: options.queryKey })
            const previousData = queryClient.getQueryData(options.queryKey)
            queryClient.setQueryData(
                options.queryKey,
                (oldData: unknown) => options.onMutate?.(oldData, mutationOptions) ?? oldData
            )
            return { previousData }
        },
